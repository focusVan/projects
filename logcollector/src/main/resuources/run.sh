...
if[ "$#" != "1" ];
    then echo "usage: $0 < req || clk || imp || win >(ignore case)"
    exit 1
fi
...
/usr/local/spark/bin/spark-submit \
    ...
    --class com.madhouse.madmax.LogCollector \
    --verbose hdfs://maddsp/madmax/apps/logCollector/logColector-assembly-0.0.1.jar -t $type